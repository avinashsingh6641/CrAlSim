...
function additiveEncrypt() {
  ...
  for (let i = 0; i < additivePlainText.length; i++) {
    ...
    if (
      additivePlainText.charAt(i) >= "a" &&
      additivePlainText.charAt(i) <= "z"
    ) {
      // console.log(additivePlainText.codePointAt(i) ,additiveKey.charCodeAt(0));
      cipherStr += String.fromCharCode(
        +(
          (+(additivePlainText.codePointAt(i) - 97) +
            +(additiveKey.charCodeAt(0) - 97)) %
          26
        ) + +97,
      );
      ...
      cipherStr += String.fromCharCode(
        +(
          (+(additivePlainText.codePointAt(i) - 65) +
            +(additiveKey.charCodeAt(0) - 65)) %
          26
        ) + +65,
      );
...
function additiveTheoryDecrypt() {
  ...
  for (let i = 0; i < additiveCipherText.length; i++) {
    ...
    if (
      additiveCipherText.charAt(i) >= "a" &&
      additiveCipherText.charAt(i) <= "z"
    ) {
      ...
      intermediate =
        (additiveCipherText.codePointAt(i) -
          97 -
          (additiveKey.charCodeAt(0) - 97)) %
        26;
      if (intermediate < 0) {
