  ...
  try {
    ...
    for (let i = 0; i < affinePlaintext.length; i++) {
      if (
        ...
        table.rows[0].insertCell().innerHTML = affinePlaintext.charAt(i);
        table.rows[1].insertCell().innerHTML =
          affinePlaintext.codePointAt(i) - 65;
        table.rows[2].insertCell().innerHTML =
...
function calculateAffineChar() {
  ...
  try {
    ...
    td1.appendChild(div2).innerHTML =
      `((${a}*${affineBMV})+${b})%26 =${affineAMV}`;
...
affinecalButton.addEventListener("click", affineSimulateFunctions);
